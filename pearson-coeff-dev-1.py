...
def CreateCellDataFrama(df):
    ...
    for _index, item in data.iterrows():
        ...
        for cellId in cellIds:
            cellProcessMask =  np.copy(processMask)
            print(_index, item["field-of-view"])
            print(cellId)
            [x1, y1, x2, y2] = getMaskBoxForInt(cellId, cellProcessMask)
            cellMask = cellProcessMask[x1 : (x2 + 1), y1 : (y2 + 1)]
